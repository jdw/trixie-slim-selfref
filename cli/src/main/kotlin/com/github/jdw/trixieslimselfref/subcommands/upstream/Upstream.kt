...
class Upstream(): CliktCommand(help = "Upstream management subcommand", printHelpOnEmptyArgs = true) {
	...
	companion object {
		private fun checkForNewChecksums() {
					...
					(response.statusCode == 200)
						.echt {
							...
							(storedChecksum == fetchedChecksum)
								...
								.doch {
									bash.run("""echo "$fetchedChecksum" > ${Glob.settings.git.branches.checksumsDir}/checksums/$arch.sha256""")
										.also { result ->
											(result.exitCode == 0)
			...
			listOf(
				Commandos(
					"cd ${Glob.settings.git.branches.checksumsDir}",
					"Failed to enter directory '${Glob.settings.git.branches.checksumsDir}'!",
					{ _ -> Glob.message("Entered directory '${Glob.settings.git.branches.checksumsDir}'!") },
					Glob.ExitCodes.FAILED_ENTER_DIR,
					{ true }
				),
				Commandos(
					Glob.settings.git.commands.countChecksumChanges,
				...
				Commandos(
					Glob.settings.git.commands.addAll,
					"Failed to run '${Glob.settings.git.commands.addAll}' in '${Glob.settings.git.branches.checksumsDir}'!",
